...
class Fileupload
{
    ...
    function do_upload($upload_path = null, $field_name = null) {
        if (empty($_FILES[$field_name]['name'])) {
            ...
            $config = [
                ...
                'allowed_types' => 'gif|jpg|png|jpeg|ico|svg|mp3|docx|pdf|',
                'max_filename'  => 5,
                'overwrite'     => false,
                'maintain_ratio' => true,
                'encrypt_name'  => false,
    ...
    public function download($id){
        if(!empty($id)){
            ...
            $imageInfo = $this->image->getRows(array('mesurementid' => $mesurementid));
            
            //file path
            $image = 'application/modules/room_facilities/assets/images/'.$imageInfo['image'];
